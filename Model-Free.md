...
First we have $G_{t:T}=r_t+\gamma r_{t+1}+...+\gamma^{T-t-1}r_{T-1}$ which is a version of MC waiting until $T$ is large enough. By performing n-step rollout combining with value functions, we have the n-step return: 
```math
\begin{aligned}
